...
func compressSessions(sessions map[string]Session) []CompressedSession {
	...
	for _, session := range sessions {
		...
		if !exists {
			...
			// If we have multiple sessions and the source port is >1024, mark it as ephemeral
			if session.SourcePort > 1024 {
...
func pruneSessions(aiResponse string) {
	...
	// Clean up stale ignore patterns (older than 5 minutes)
	var activePatterns []IgnorePattern
	for _, pattern := range ignorePatterns {
		if now.Sub(pattern.LastSeen) <= 5*time.Minute {
			activePatterns = append(activePatterns, pattern)
		} else if *debugMode {
			logger.Debugf("Aging out ignore pattern: %s:%s (last seen %v ago)",
				pattern.Type, pattern.Value, now.Sub(pattern.LastSeen))
		}
	}
	ignorePatterns = activePatterns
	...
	if len(analysis.Ignore) > 0 {
		...
		for _, ignore := range analysis.Ignore {
			...
			if !existingPatterns[patternKey] {
				...
				if *debugMode {
					logger.Debugf("Added new ignore pattern: %s:%s", ignore.Type, ignore.Value)
				}
			}
		}
	}
	...
	// Log current state of ignore patterns
	if *debugMode {
		logger.Debug("\nCurrent Ignore Patterns:")
		for _, pattern := range ignorePatterns {
			logger.Debugf("- %s:%s", pattern.Type, pattern.Value)
		}
	}
	...
	for i, pattern := range ignorePatterns {
		if pattern.Type == "port" {
			...
			if session.SourcePort == port || session.DestinationPort == port {
				...
				// Update LastSeen for this pattern
				ignorePatterns[i].LastSeen = time.Now()
				return true
			}
		}
	}
